    ...
    public class RatingCrossValidationSplit : ISplit<IRatings>
    {
        ...
        public RatingCrossValidationSplit(IRatings ratings, uint num_folds)
        {
            ...
            for (int i = 0; i < num_folds; i++)
                ...
                else if (ratings is IFeaturedRatings)
                {
                    Train.Add(new FeaturedRatingsProxy((IFeaturedRatings)ratings, train_indices[i]));
                    Test.Add(new FeaturedRatingsProxy((IFeaturedRatings)ratings, test_indices[i]));                   
                }
                else
                {
